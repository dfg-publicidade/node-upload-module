...
import { CloudImageUploadConfig, CloudUploadConfig, S3ImageUpload } from '../../src';
...
describe('s3ImageUpload.ts', (): void => {
    const config: any = {
        upload: {
            aws: {},
            fileUpload: {},
    ...
    before(async (): Promise<void> => {
        ...
        exp.post('/image1', async (req: Request, res: Response): Promise<void> => {
            try {
                const imageUpload: S3ImageUpload = new S3ImageUpload(config.upload, {
    ...
    it('2. constructor', async (): Promise<void> => {
        expect((): void => {
            new S3ImageUpload(config.upload, {} as CloudUploadConfig);
    ...
    it('13. save', async (): Promise<void> => {
        const buffer: Buffer = await fs.readFile(__dirname + '/../test.png');

        const s3ImageUpload: S3ImageUpload = new S3ImageUpload(config.upload, {
            bucket: process.env.AWS_S3_TEST_BUCKET,
            name: 'file',
            prefix: 'f',
            dir: 'test/',
        ...
        const json: any = await s3ImageUpload.save('ref-1', '.png', buffer);
